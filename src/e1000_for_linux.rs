...
use kernel::{
    ...
    pci,
    pci::MappedResource, spinlock_init, sync::{Arc, ArcBorrow, CondVar, SpinLock, UniqueArc}, PointerWrapper,
};
...
impl net::DeviceOperations for E1000Driver {
    ...
    fn open(dev: &Device, data: &NetData) -> Result {
        ...
        let irq = irq::Registration::try_new(
            data.irq,
            intrdata,
            irq::flags::SHARED,
            fmt!("e1000")
    ...
    fn start_xmit(
        ...
        if len < 0 {
            pr_warn!("Failed to sen transmit the skbuff packet: {}", len);
...
impl kernel::Module for RustE1000dev {
    fn init(name: &'static CStr, module: &'static ThisModule) -> Result<Self> {
        ...
        // From linux kernel linux/log.h
        unsafe {
            bindings::rust_e1000_log();
        }
